...
describe('Thing API:', function () {
  ...
  describe('PATCH /api/things/:id', function () {
    ...
    beforeEach(function (done) {
      (0, _supertest2.default)(app).patch('/api/things/' + newThing._id).send([{ op: 'replace', path: '/name', value: 'Patched Thing' }, { op: 'replace', path: '/info', value: 'This is the patched thing!!!' }]).expect(200).expect('Content-Type', /json/).end(function (err, res) {
        if (err) {
          return done(err);
        }
  ...
  describe('DELETE /api/things/:id', function () {
    it('should respond with 204 on successful removal', function (done) {
      (0, _supertest2.default)(app).delete('/api/things/' + newThing._id).expect(204).end(function (err) {
        if (err) {
          return done(err);
        }
        done();
    ...
    it('should respond with 404 when thing does not exist', function (done) {
      (0, _supertest2.default)(app).delete('/api/things/' + newThing._id).expect(404).end(function (err) {
        if (err) {
          return done(err);
        }
        done();
...
//# sourceMappingURL=thing.dontrun-integration.js.map
